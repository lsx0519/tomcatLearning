...
public abstract class ContainerBase extends LifecycleMBeanBase
     ...
     *  child Containers
     *  
     *  1.StandardHost[localhost]
     *  2.StandardContext[/docs]
     *  
     */
    @Override
    public void addChild(Container child) {
    	//全局Globals.IS_SECURITY_ENABLED=false
    ...
    /**
     * 1.StandardEngine[Catalina].StandardHost[localhost]
     * 2.StandardEngine[Catalina].StandardHost[localhost].StandardContext[/docs]
     * @param child
     */
        ...
        synchronized(children) {
            if (children.get(child.getName()) != null)
                throw new IllegalArgumentException("addChild:  Child name '" +
                                                   child.getName() +
                                                   "' is not unique");
            //设置子容器的父容器为当前对象，StandardEngine[Catalina]
            //设置子容器的父容器为当前对象，StandardEngine[Catalina].StandardHost[localhost]
    ...
    protected void initInternal() throws LifecycleException {
        ...
        startStopExecutor = new ThreadPoolExecutor(
                getStartStopThreadsInternal(),//核心线程池大小
                getStartStopThreadsInternal(),//最大线程池大小
                10, //keepAliveTime
                TimeUnit.SECONDS,//keepAliveTime时间单位
                startStopQueue,//阻塞任务队列
                new StartStopThreadFactory(getName() + "-startStop-"));//新建线程工厂
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // 启动我们的子容器,FutureTask模式启动，嵌套方式等待子容器启动好。
        Container children[] = findChildren();
        List<Future<Void>> results = new ArrayList<>();
        for (int i = 0; i < children.length; i++) {
        	//底层默认实现是FutureTask，具有Runnable, Future的功能
        	//new StartChild(children[i])线程池回去执行Callable的run方法
        	//startStopExecutor的submit是具有转换功能
        	//AbstractExecutorService的newTaskFor方法：将Callable类型转成FutureTask类型，RunnableFuture是FutureTask的父类接口
        	//1.FutureTask的run方法会执行Callable的call方法。
        	//2.FutureTask会通过线程的worker模式，信息worker任务，构建本身线程，执行run方法
        	//3.基于以上，StartChild实现Callable的方法就能顺利执行StartChild的call的方法
        	//4.就这样子容器开始启动，子容器早就已经准备好了
            //children在Digester分析server.xml时候就初始化好了
		 ...
		 *  Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[RequestInfoExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[async3]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[numberwriter]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[async2]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[async1]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[jsp]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[async0]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[bytecounter]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[ServletToJsp]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[ChatServlet]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[RequestParamExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[RequestHeaderExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[default]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[CookieExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[CompressionFilterTestServlet]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[HelloWorldExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[SessionExample]]
			Pipeline[StandardEngine[Catalina].StandardHost[localhost].StandardContext[/examples].StandardWrapper[stock]]
			...
			Pipeline[StandardEngine[Catalina]]
         */
        
        // 如果有的话，就在pipeline启用这些值
        if (pipeline instanceof Lifecycle)
        	System.out.println(pipeline.toString()+":"+pipeline.getBasic());
    ...
    private static class StartChild implements Callable<Void> {
        ...
        public Void call() throws LifecycleException {
        	//此时子容器生命周期是创建状态，LifecycleState.NEW，在LifecycleBase中start方法他遇到此状态他回去执行ContainerBase的initInternal()方法，
        	//容器通过startStopQueue去提交子容器启动，每个子容器刚开始都是LifecycleState.NEW状态
            child.start();
            return null;
        }
    }

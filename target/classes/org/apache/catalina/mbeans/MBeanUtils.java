...
public class MBeanUtils {
    ...
    public static DynamicMBean createMBean(ContextResource resource)
        ...
        //ManagedBean[name=ContextResource, className=org.apache.catalina.mbeans.ContextResourceMBean, description=Representation of a resource reference for a web application, group=Resources, type=org.apache.tomcat.util.descriptor.web.ContextResource]
        ManagedBean managed = registry.findManagedBean(mname);
        if (managed == null) {
            Exception e = new Exception("ManagedBean is not found with "+mname);
            throw new MBeanException(e);
        ...
        if (domain == null)
            domain = mserver.getDefaultDomain();
        //创建一个动态的管理bean 
        //ContextResource[name=UserDatabase, description=User database that can be updated and saved, type=org.apache.catalina.UserDatabase, auth=Container, scope=Shareable]

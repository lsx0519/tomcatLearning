...
public class StandardEngine extends ContainerBase implements Engine {
     ...
     * 获得已配置的域，并在没有设置显式配置时提供默认的域实现。默认是使用LockOutRealm防止试图通过暴力攻击猜测用户密码
    ...
    protected void initInternal() throws LifecycleException {
    	...
    	// 在尝试启动一个服务之前，请确定该服务已经存在。如果需要，这将创建默认的NullRealm。
        getRealm();
        // 启动一个线程池ThreadPoolExecutor去执行启动和停止中要用到的，线程池大小默认是1
        super.initInternal();
    }
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // 启动容器，子容器启动逻辑在里面
        // ContainerBase.startInternal()->startStopExecutor.submit()
        super.startInternal();
    }

...
public abstract class AbstractProtocol<S> implements ProtocolHandler,
     ...
     */
    /**
     * 在这个类中不需要维护状态或检查有效的转换。预计连接器将维护状态并防止无效的状态转换。注册-关联bean
     */
    @Override
    public void init() throws Exception {
        ...
        //org.apache.tomcat.util.net.NioEndpoint@74a10858
    ...
    protected abstract static class AbstractConnectionHandler<S,P extends Processor>
        ...
        protected void register(AbstractProcessor processor) {
        	//获取当前协议所处的容器：Catalina
            if (getProtocol().getDomain() != null) {
                synchronized (this) {
                    try {
                        long count = registerCount.incrementAndGet();
                        //封装一个请求信息对象，封装的是Request对象
                        RequestInfo rp =
                            processor.getRequest().getRequestProcessor();
                        //RequestGroupInfo嵌入到RequestInfo

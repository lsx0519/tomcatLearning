...
public class Http11Processor extends AbstractProcessor {
    ...
    public SocketState process(SocketWrapperBase<?> socketWrapper)
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() &&
            ...
            // Upgrade:检测http协议及其他协议是否可使用更高的版本进行通信，其参数值可用来指定一个完全不同的通信协议
            // 客户端请求:MT /index.htm HTTP/1.1  Upgrade: TLS/1.0  Connection: Upgrade
            // 服务器响应:HTTP/1.1 101 Switching Protocols Upgrade: TLS/1.0, HTTP/1.1 Connection: Upgrade
            ...
            if (!getErrorState().isError()) {
                try {
                	//请求第三阶段,CoyoteAdapter.service嵌入Request，Response对象
